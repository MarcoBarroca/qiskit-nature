...
mat_names_arr = [
    ...
    "ALPHA SCF DENSITY",
    "BETA SCF DENSITY",
    "ALPHA MP FIRST ORDER DENSITY",
    "BETA MP FIRST ORDER DENSITY",
    "ALPHA MP2 DENSITY",
    "BETA MP2 DENSITY",
    "ALPHA MP3 DENSITY",
    "BETA MP3 DENSITY",
    "ALPHA MP4 DENSITY",
    "BETA MP4 DENSITY",
    "ALPHA CI ONE-PARTICLE DENSITY",
    "BETA CI ONE-PARTICLE DENSITY",
    "ALPHA CI DENSITY",
    "BETA CI DENSITY",
    "ALPHA QCI/CC DENSITY",
    "BETA QCI/CC DENSITY",
    "ALPHA DENSITY CORRECT TO SECOND ORDER",
...
def makegauinp(
    ...
    if dofock is False:
        if haveorbs:
            fi.write("guess=(copychk,only)")
        else:
            fi.write("guess=(*none*,only)")
    elif dofock is True:
        fi.write("guess=read scf=(novaracc,maxcyc=-1)")
    elif dofock.upper() == "DENSITY":
        fi.write("guess=copychk scf=(novaracc,maxcyc=-1)")
    elif dofock.upper() == "SCF":
        fi.write("scf=(novaracc)")
    elif dofock.upper() == "SCFREAD":
        fi.write("guess=read scf=(novaracc)")
    else:
        raise TypeError
    if aotype is not None:
        fi.write(" scf=conventional ")
        if aotype == 0 or aotype == "regular" or aotype == "noraff":
            fi.write("noraff")
        else:
            fi.write("int=raf%d" % aotype)
    if motran == "partial":
        fi.write(" tran=iabc")
    elif motran == "full":
        fi.write(" tran=full")
    if window is not None:
        fi.write(" window=" + str(window))
...
def rungau(matfi, matfo, program="g16", progargs=[], debug=False, toutput=None, **kwargs):
    ...
    try:
        ...
        pass
    if itemp is None:
        fi = subprocess.DEVNULL
    else:
        fi = open(itemp, mode="r")
...
class MatEl(object):
    ...
    def addobj(self, obj):
        name = obj.name.upper()
        if name in mat_names_synonyms:
            name = mat_names_synonyms[name]
        obj.name = name
    ...
    def delobj(self, namei):
        ...
        if name in mat_names_synonyms:
            name = mat_names_synonyms[name]
        if name in self.__MATLIST:
            del self.__MATLIST[name]

    def scalar(self, namei, *val):
        name = namei.upper()
        if name in scalar_synonyms:
            name = scalar_synonyms[name]
        assert name in scalar_names
        if len(val) > 0:
            self.__GSCAL[scalar_names[name]] = val[0]
        return self.__GSCAL[scalar_names[name]]

    def set_scalars(self, **kwargs):
        for name in kwargs:
            self.scalar(name, kwargs[name])
    ...
    def read(self, fname, check_status=False):
        ...
        gotone = True
        while gotone:
            cbuf, ni, nr, ntot, lenbuf, n1, n2, n3, n4, n5, asym, nri, eof = qcmio.rd_labl(
                self.unit, self.fversion
            )
            ...
            if gotone:
                ...
                if (ni >= 1) and (nr == 0):
                    ...
                    if nri == 1:
                        arr = qcmio.rd_rbuf(self.unit, nr * ntot, nr * lenbuf)
                    else:
                        arr = qcmio.rd_cbuf(self.unit, nr * ntot, nr * lenbuf)
                    myobj = qco.OpMat(cbuf, arr, asym=asym, nelem=nr, dimens=dimens)
                elif qcmio.aoints(cbuf):
                    if nr == 1:
                        arr = qcmio.rd_2e1(self.unit, lr, ntot, lenbuf)
                    else:
                ...
                if cbuf == GSNAME:
                    ...
                    self.__MATLIST[cbuf] = myobj
        qcmio.close_matf(self.unit)
        if check_status:
            assert self.scalar("JOB STATUS") == 1.0
    ...
    def write(self, fname):
        ...
        for lab in mat_names_arr:
            if lab == GSNAME:
                ...
                if lab in self.__MATLIST:
                    self.__MATLIST[lab].write(self.unit, WLENBUF)
        for lab in sorted(self.__MATLIST):
            if not lab in mat_names:
                self.__MATLIST[lab].write(self.unit, WLENBUF)
    ...
    def update(self, matfi=None, matfo=None, check_status=True, doinit=False, **kwargs):
        ...
        try:
            ...
            print(
                "Gaussian failed; retaining matinp",
                matinp,
                "matout",
                matout,
                "input",
        ...
        if itemp is not None and ("tinput" not in kwargs or kwargs["tinput"] is None):
            os.unlink(itemp)
        if "toutput" not in kwargs or kwargs["toutput"] is None:
            os.unlink(otemp)
        if matfi is None:
            os.unlink(matinp)
        if matfo is None:
    ...
    def load_head(
        ...
        c=None,
        atmwgt=None,
        labfil="Gaussian matrix elements",
        ...
        if icgu is None:
            if iopcl == 6:
                ...
                self.icgu = 10 * ((iopcl % 4) // 2) + (iopcl % 2) + 111
        else:
            self.icgu = icgu
        self.nfc = nfc
        self.nfv = nfv
        ...
        if iattyp is None:
            self.iattyp = np.zeros((natoms,), dtype=INTTYPE)
        else:
            assert len(iattyp) == self.natoms
            self.iattyp = np.array(iattyp, dtype=INTTYPE)
        if atmchg is None:
            self.atmchg = np.array(self.ian, dtype="float64")
        else:
        ...
        if atmwgt is None:
            self.atmwgt = np.array(
                [defatw[min(max(ia, 0), maxan)] for ia in self.ian], dtype="float64"
            )
        else:
        ...
        self.__MATLIST = {}
        for i in range(len(self.__GSCAL)):
            self.__GSCAL[i] = 0.0e0
        if atznuc is None:
            znuc = self.atmchg
        else:
