...
def _lenarray(d):
    if len(d) == 5:
        l = (
            d[4]
            * qcmio.lind4(
                False, d[0], d[1], d[2], d[3], 0, abs(d[0]), abs(d[1]), abs(d[2]), abs(d[3])
            )[0]
            + 1
        )
    elif len(d) == 4:
...
def _makeindx(dimens, asym, args):
    if len(dimens) >= 5:
        ...
        return qcmio.lind3(True, dimens[0], dimens[1], dimens[2], asym, args[0], args[1], args[2])
    elif len(dimens) == 2:
        return qcmio.lind2(True, dimens[0], dimens[1], asym, args[0], args[1])
    else:
        return (args[0] - 1, 1.0e0)
...
def _makeindxc(dimens, asym, args):
    if len(dimens) >= 5:
        ...
        return qcmio.lind3(
            True, dimens[0], dimens[1], dimens[2], asym, args[2] + 1, args[1] + 1, args[0] + 1
        )
    elif len(dimens) == 2:
        return qcmio.lind2(True, dimens[0], dimens[1], asym, args[1] + 1, args[0] + 1)
    else:
        return (args[0], 1.0e0)
...
def printlab(cbuf, ni, nr, nri, ntot, lenbuf, n1, n2, n3, n4, n5, asym, doinp=False, **kwargs):
    if doinp:
        ...
        if lenbuf > 0:
            print("LenBuf=%8d" % lenbuf, end=" ", **kwargs)
...
def print1d(comp, type, wid, label, arr, doinp=False, **kwargs):
    ...
    ndone = 0
    while i < arr.size:
        if (abs(arr[i]) >= 1.0e-12) or not comp:
            if ndone == 0:
...
def print2e(cbuf, n, r, doinp=False, **kwargs):
    if doinpprt(cbuf, r, doinp=False, **kwargs):
        return
    if re.match("^REG", cbuf):
        lab = "Int"
    else:
        lab = "R1"
    if len(np.shape(r)) == 1:
        ...
        for j in range(i + 1):
            for k in range(i + 1):
                ...
                for l in range(llim):
                    ...
                    if nr == 1:
                        ri[0] = r[ijkl]
                        doit = doit or (abs(ri[0]) >= 1.0e-12)
                    else:
                        for x in range(nr):
                            ri[x] = r[ijkl, x]
                            doit = doit or (abs(ri[x]) >= 1.0e-12)
                    if doit:
                        str = "I=%3i J=%3i K=%3i L=%3i %s=%20.12e" % (
                            i + 1,
                            j + 1,
                            k + 1,
                            l + 1,
                        ...
                        if nr > 1:
                            str = str + " R2=%20.12e" % ri[1]
                        if nr > 2:
                            str = str + " R3=%20.12e" % ri[2]
                        str = str.replace("e", "D")
                        print(str, **kwargs)
...
class OpMat(object):
    ...
    def __init__(self, name, array, nelem=1, type=None, asym=False, dimens=None):
        ...
        if type is None:
            if self.array.dtype == np.int32:
                self.type = "i"
            elif self.array.dtype == np.int64:
                self.type = "i"
            elif self.array.dtype == np.float64:
                self.type = "d"
            elif self.array.dtype == np.complex128:
                self.type = "c"
            else:
                raise TypeError
        ...
        if asym:
            ...
            self.asym = False
        if dimens is None:
            self.dimens = (self.array.size / self.nelem,)
        elif not isinstance(dimens, tuple):
            raise TypeError
        else:
            self.dimens = dimens
    ...
    def print_mat(self, wid=1, doinp=False, **kwargs):
        ...
        if doinpprt(name, self.array, doinp=False, **kwargs):
            return
        printlab(name, ni, nr, nri, ntot, 0, n1, n2, n3, n4, n5, asym, **kwargs)
        if qcmio.aoints(name):
            print2e(self.name, self.dimens[3], self.array, **kwargs)
        elif len(self.dimens) == 1:
            if re.match("GAUSSIAN SCALARS", self.name):
                print1d(True, self.type, 5, " ", self.array, **kwargs)
            else:
                print1d(False, self.type, wid, " ", self.array, **kwargs)
        elif len(self.dimens) == 2:
            if self.dimens[0] < 0:
                ltout(" ", self.dimens[1], self.array, 0, 0, **kwargs)
            else:
                sqout(" ", self.dimens[0], self.dimens[1], self.array, 0, 0, **kwargs)
            ...
            if self.dimens[0] < 0 and self.dimens[1] > 0 and allpos:
                for im in range(nmat):
                    ltout(name, self.dimens[1], self.array, 0, im + 1, **kwargs)
            elif self.dimens[0] > 0 and self.dimens[1] > 0 and allpos:
                for im in range(self.dimens[2]):
                    sqout(name, self.dimens[0], self.dimens[1], self.array, 0, im + 1, **kwargs)
            elif (
                (len(self.dimens) >= 4)
                and (self.dimens[0] == -self.dimens[1])
                and (self.dimens[2] == -self.dimens[3])
            ):
                nmat = (self.dimens[3] * (self.dimens[3] + 1)) // 2
                if len(self.dimens) >= 5:
                    nmat = self.dimens[4] * nmat
                for im in range(nmat):
                    ltout(name, self.dimens[1], self.array, 0, im + 1, **kwargs)
            else:
                print1d(False, self.type, 1, " ", self.array, **kwargs)
        else:
    ...
    def expand(self):
        ...
        if qcmio.aoints(self.name):
            if self.dimens[0] < 0:
                ...
                if self.nelem == 1:
                    narr = qcmio.expao1(n, self.array)
                else:
                    narr = qcmio.expaon(n, self.array)
            else:
                narr = self.array
        else:
            narr = np.empty(d, dtype=type(self.array[0]))
            for i in np.ndindex(*d):
                narr[i] = self.get_elemc(*i)
    ...
    def wr_lbuf(self, iu, lenbuf):
        ...
        if self.type == "i":
            qcmio.wr_ibuf(iu, lenbx, self.array)
        elif self.type == "c":
            qcmio.wr_cbuf(iu, lenbx, self.array)
        else:
            qcmio.wr_rbuf(iu, lenbx, self.array)
